...
{
    public bool VerifySignature(PublicKey signer) => Ed25519.Ed25519.Verify(signer, Event.ToByteArray(), Signature);

    private static Lazy<Dictionary<string, (Type type, MessageDescriptor descriptor)>> _lazyTypeDictionary = new Lazy<Dictionary<string, (Type type, MessageDescriptor descriptor)>>(() =>
    {
    ...
    public static SignedEvent Deserialize(byte[] bytes)
    {
        ...
        var (eventType, descriptor) = _lazyTypeDictionary.Value[serializedSignedEvent.Type];
