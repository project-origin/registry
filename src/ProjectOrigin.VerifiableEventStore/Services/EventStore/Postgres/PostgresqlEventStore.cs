...
public sealed class PostgresqlEventStore : IEventStore, IDisposable
{
    ...
    public PostgresqlEventStore(IOptions<PostgresqlEventStoreOptions> options, BlockSizeCalculator blockSizeCalculator)
    {
    ...
    public async Task<ImmutableLog.V1.Block?> GetBatchFromTransactionHash(TransactionHash transactionHash)
    {
        using var connection = _dataSource.CreateConnection();
    ...
    public async Task<IList<VerifiableEvent>> GetEventsForBatch(BatchHash batchHash)
    {
        using var connection = _dataSource.CreateConnection();

        var batchInfo = await connection.QuerySingleAsync<(long FromTransaction, long ToTransaction)>(
              "SELECT from_transaction, to_transaction FROM blocks WHERE block_hash = @blockHash",
              new { blockHash = batchHash.Data });

        var events = await connection.QueryAsync<VerifiableEvent>(
    ...
    public async Task<IList<VerifiableEvent>> GetEventsForEventStream(Guid streamId)
    {
    ...
    public async Task<(ImmutableLog.V1.BlockHeader, IList<TransactionHash>)> CreateNextBatch()
    {
        using var connection = _dataSource.CreateConnection();
        ...
        try
        {
            // create table lock on blocks table to prevent concurrent batch creation
            ...
            var transactions = (await connection.QueryAsync<VerifiableEvent>(
            ...
            return (blockHeader, transactions.Select(x => x.TransactionHash).AsList());
        }
        catch
        {
    ...
    public async Task FinalizeBatch(BatchHash hash, ImmutableLog.V1.BlockPublication publication)
    {
        using var connection = _dataSource.CreateConnection();
    ...
    private record DBBlock(byte[] BlockHash, byte[] PreviousHeaderHash, byte[] PreviousPublicationHash, byte[] MerkleRootHash, DateTimeOffset CreatedAt, long FromTransaction, long ToTransaction, byte[]? Publication);
}
