...
public class BatchProcessorJobTests
{
    ...
    public async Task Will_Finalize_Full_Batch(int numberOfEvents, int batchExponent, int expectedBatches)
    {
        ...
        // var blockChainConnector = new Mock<IBatchPublisher>();
        ...
        // {
        //     var @event = new VerifiableEvent(fixture.Create<TransactionHash>(), Guid.NewGuid(), 0, fixture.Create<byte[]>());
        //     await eventStore.Store(@event);
        // }

        // var job = new BatchProcessorJob(blockChainConnector.Object, eventStore, iTransactionService.Object);
        // blockChainConnector.Setup(obj => obj.P.PublishBytes(It.IsAny<byte[]>())).Returns(Task.FromResult(new TransactionReference(transactionId)));
